 ...
 * Statistics are returned in the form of a {@link ResultsTable}
 * Multiple images are not directly supported, as we have never had a use for it.
...
public class StatsQuery {
    ...
    /**
     * allows to select the name of the statistic to export. These are the same names as in the Imaris GUI
     * **Minus** the channel or image (eg. do not enter "Intensity Sum" Ch1=1 Img=1, just "Intensity Sum")
     * Use {@link StatsQuery#selectChannels(List)} and {@link StatsQuery#selectChannel(Integer)} to specify channels
     ...
     * the names, channels and timepoints that were requested. If they all match
     * @return
     * @throws Error
     */
    public ResultsTable get( ) throws Error {
        ...
        for ( int i = 0; i < this.stats.mIds.length; i++ ) {
            ...
            if ( matchesName && matchesChannel && matchesID && matchesTime ) {
                ...
                results.addValue( "Channel", this.stats.mFactors[ channelIdx ][ i ] );
                results.addValue( "Timepoint", this.stats.mFactors[ timeIdx ][ i ] );
                results.addValue( "ID", this.stats.mIds[ i ] );
                results.addValue( "Category", this.stats.mFactors[ catIdx ][ i ] );
